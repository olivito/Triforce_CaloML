...
from triforce_helper_functions_features import eval
import numpy as np
...
class Analyzer():
    ...
    def analyze(self, tools, testLoader, out_file):
        ...
        for data in testLoader:
            ...
            if (classifier != None): classifier_outputs.append(eval(classifier, features, ys))
            else: classifier_outputs.append((0,0,0,0,0,0))
            if (regressor != None): regressor_outputs.append(eval(regressor, features, energies))
            else: regressor_outputs.append((0,0,0,0,0,0))
            if (GAN != None): GAN_outputs.append(eval(GAN, features, ys))
