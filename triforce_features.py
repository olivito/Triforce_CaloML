...
import numpy as np
import h5py as h5
import Loader.loader_features as loader
import shutil
from triforce_helper_functions_features import *
import Options
...
optionsFileName = "default_options_features"
...
regressor_meandiff_history_test = []
regressor_sigmadiff_history_test = []
...
calculate_loss_per = 100
#calculate_loss_per = 20000
...
def update_test_loss(epoch_end):
    ...
    for data in testLoader:
        features, ys, energies = data
        features, ys, energies = Variable(features.cuda()), Variable(ys.cuda()), Variable(energies.cuda())
        if (classifier != None):
            classifier_test_output = eval(classifier, features, ys)
            classifier_test_loss += classifier_test_output[0]
            classifier_test_accuracy += classifier_test_output[1]
        if (regressor != None):
            loss, acc, _, _, mean, regressor_test_sigmadiff = eval(regressor, features, energies)
            regressor_test_loss += loss
            regressor_test_meandiff += mean
#            regressor_test_loss += eval(regressor, features, energies)[0]
        if (GAN != None):
            GAN_test_loss += eval(GAN, features, ys)[0]
            GAN_test_accuracy += eval(GAN, features, ys)[1]
        n_test_batches += 1
    ...
    regressor_meandiff_history_test.append(regressor_test_meandiff)
    regressor_sigmadiff_history_test.append(regressor_test_sigmadiff)
    ...
    # # -> don't compute test loss until end of epoch to avoid slowdown from loading files
    # if (not epoch_end):
    #     total_test_loss = 0
...
for epoch in range(options['nEpochs']):
    ...
    for i, data in enumerate(trainLoader):
        ...
        features, ys, energies = Variable(features.cuda()), Variable(ys.cuda()), Variable(energies.cuda())
        if (classifier != None):
            classifier_training_loss += train(classifier, features, ys)[0]
            classifier_training_accuracy += train(classifier, features, ys)[1]
        if (regressor != None):
            regressor_training_loss += train(regressor, features, energies)[0]
        if (GAN != None):
            GAN_training_loss += train(GAN, features, ys)[0]
            GAN_training_accuracy += train(GAN, features, ys)[1]
...
if (regressor != None): 
    ...
    out_file.create_dataset("regressor_meandiff_history_test", data=np.array(regressor_meandiff_history_test))
    out_file.create_dataset("regressor_sigmadiff_history_test", data=np.array(regressor_sigmadiff_history_test))
